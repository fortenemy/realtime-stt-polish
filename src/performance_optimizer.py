...
class PerformanceOptimizer:
    ...
    def _check_optimization_triggers(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            if self.optimize_memory and metrics.is_memory_critical:
                logger.warning(
                    f"⚠️ Krytyczny poziom pamięci: {metrics.memory_percent:.1f}%"
                )
            ...
            if (
                ...
                and metrics.gpu_memory_total_mb
                and metrics.gpu_memory_used_mb / metrics.gpu_memory_total_mb > 0.9
            ):
    ...
    def get_average_metrics(self, last_n: int = 10) -> Optional[PerformanceMetrics]:
        ...
        avg_threads = sum(m.active_threads for m in recent_metrics) / len(
            recent_metrics
        )
        ...
        avg_gpu_total = (
            ...
            if recent_metrics and recent_metrics[0].gpu_memory_total_mb
            else None
        )
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        if current_metrics.memory_percent > 70:
            recommendations.append(
                "💾 Rozważ zamknięcie innych aplikacji aby zwolnić pamięć"
            )

        if current_metrics.memory_percent > 85:
            recommendations.append(
                "🚨 Krytyczny poziom pamięci - restart aplikacji zalecany"
            )

        # CPU recommendations
        if current_metrics.cpu_percent > 70:
            recommendations.append(
                "⚡ Wysokie obciążenie CPU - rozważ użycie mniejszego modelu STT"
            )
        ...
        if current_metrics.gpu_memory_used_mb and current_metrics.gpu_memory_total_mb:
            gpu_usage = (
                current_metrics.gpu_memory_used_mb / current_metrics.gpu_memory_total_mb
            )

            if gpu_usage > 0.8:
                recommendations.append(
                    "🔥 Wysokie użycie pamięci GPU - rozważ mniejszy model"
                )

            if gpu_usage > 0.95:
                recommendations.append(
                    "🚨 Krytyczny poziom pamięci GPU - zmniejsz batch size"
                )

        # Thread recommendations
        if current_metrics.active_threads > 20:
            recommendations.append(
                "🧵 Duża liczba wątków - możliwa optymalizacja threading"
            )
    ...
    def cache_model(self, model_name: str, model_object):
        ...
        try:
            ...
            if model_size_mb > self.cache_limit_mb:
                logger.warning(
                    f"⚠️ Model {model_name} zbyt duży dla cache: {model_size_mb}MB"
                )
                return False
    ...
    def _cleanup_cache_if_needed(self, needed_mb: float):
        ...
        if current_size + needed_mb > self.cache_limit_mb:
            ...
            sorted_models = sorted(
                self.cached_models.items(), key=lambda x: x[1]["last_used"]
            )
