...
class WhisperSTTEngine:
    ...
    def __init__(
        ...
        """
        self.model_name = (
            model_name.value if isinstance(model_name, WhisperModel) else model_name
        )
        ...
        logger.info(
            f"🤖 WhisperSTTEngine inicjalizowany: model={self.model_name}, "
    ...
    def transcribe_audio(
        ...
        try:
            ...
            if self.previous_text and decode_options.get(
                "condition_on_previous_text", True
            ):
                decode_options["initial_prompt"] = self.previous_text[
                    -200:
                ]  # Ostatnie 200 znaków
            ...
            logger.debug(
                f"🎤 Transkrypcja: '{transcription_result.text}' "
...
class PolishOptimizedSTT(WhisperSTTEngine):
    ...
    def __init__(self, **kwargs):
        ...
        logger.info(
            "🇵🇱 PolishOptimizedSTT zainicjalizowany z optymalizacjami dla polskiego"
        )
    ...
    def transcribe_audio(
        ...
        if result and result.text:
            ...
            if processed_text != original_text:
                logger.debug(
                    f"🇵🇱 Polish post-processing: '{original_text}' -> '{processed_text}'"
                )
