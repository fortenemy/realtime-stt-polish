...
class RealtimeSTTPipeline:
    ...
    def __init__(
        ...
        logger.info(f"🚀 RealtimeSTTPipeline zainicjalizowany: "
    ...
    def _handle_silence_chunk(self, current_time: float):
        ...
        if (self.current_segment_start is not None and 
            ...
            if segment_duration >= self.min_segment_duration:
                ...
                logger.debug(f"🗑️ Odrzucenie krótkiego segmentu: {segment_duration:.2f}s")
                self._discard_current_segment()
    
    def _finalize_current_segment(self):
        """Finalizuj bieżący segment mowy"""
        if (self.current_segment_start is None or 
            not self.current_segment_audio):
            return
        ...
        logger.info(f"🎯 Segment #{self.total_segments}: {segment.duration:.2f}s, "
