...
class ExportManager:
    ...
    def __init__(self):
        ...
        self.supported_formats = ["txt", "json", "csv", "srt", "vtt", "xml", "docx"]
    ...
    def _export_txt(
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            if include_metadata:
                ...
                f.write(
                    f"Całkowity czas: {self._calculate_total_duration(segments):.2f}s\n"
                )
                f.write("\n")
    ...
    def _export_xml(
        ...
        if include_metadata:
            ...
            ET.SubElement(metadata, "total_duration").text = str(
                self._calculate_total_duration(segments)
            )
        ...
        for i, segment in enumerate(segments, 1):
            ...
            if include_confidence and segment.transcription:
                ...
                ET.SubElement(transcription_elem, "confidence").text = str(
                    segment.transcription.confidence
                )
                ET.SubElement(transcription_elem, "processing_time").text = str(
                    segment.transcription.processing_time
                )
                ET.SubElement(transcription_elem, "language").text = (
                    segment.transcription.language
                )
    ...
    def _export_docx(
        ...
        try:
            ...
            logger.error(
                "❌ python-docx nie zainstalowany. Użyj: pip install python-docx"
            )
    ...
    def _calculate_total_duration(self, segments: List[SpeechSegment]) -> float:
        ...
        return max(segment.end_time for segment in segments) - min(
