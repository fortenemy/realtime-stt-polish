...
class STTGuiApplication:
    ...
    def create_menu(self):
        ...
        file_menu.add_command(
            label="Zapisz transkrypcję...", command=self.save_transcription
        )
        file_menu.add_command(
            label="Wczytaj plik audio...", command=self.load_audio_file
        )
    ...
    def create_control_panel(self, parent):
        ...
        control_frame.grid(
            row=0, column=0, rowspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(0, 10)
        )
    ...
    def create_audio_level_indicator(self, parent):
        ...
        self.audio_level_bar = ttk.Progressbar(
            level_frame, variable=self.audio_level_var, maximum=100, length=150
        )
        self.audio_level_bar.grid(row=0, column=0, pady=5)

        # VAD indicator
        self.vad_indicator = tk.Label(
            level_frame, text="🔇 Cisza", background="lightgray"
        )
    ...
    def create_transcription_area(self, parent):
        ...
        self.transcription_text = scrolledtext.ScrolledText(
            trans_frame, wrap=tk.WORD, width=50, height=20, font=("Arial", 11)
        )
        ...
        ttk.Button(button_frame, text="Wyczyść", command=self.clear_transcription).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Kopiuj", command=self.copy_transcription).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Zapisz", command=self.save_transcription).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Checkbutton(
            button_frame, text="Pewność", variable=self.show_confidence_var
        ).pack(side=tk.RIGHT, padx=5)
        ttk.Checkbutton(button_frame, text="Czas", variable=self.show_timing_var).pack(
    ...
    def create_status_bar(self, parent):
        ...
        self.status_bar = ttk.Label(
            parent, text="Gotowy do nagrywania", relief=tk.SUNKEN
        )
        ...
        try:
            ...
            if self.show_timing_var.get():
                timestamp = time.strftime(
                    "%H:%M:%S", time.localtime(segment.start_time)
                )
    ...
    def update_speech_stats(self, segment: SpeechSegment):
        ...
        # Średnia pewność
        confidences = [
            s.transcription.confidence
            for s in self.transcriptions
    ...
    def update_audio_level(self, level: float):
        ...
        level_percent = max(
            0, min(100, (level + 60) * 100 / 60)
        )  # -60dB to 0dB -> 0% to 100%
            ...
            for segment in self.transcriptions:
                if segment.text:
                    ...
                    if segment.transcription:
                        f.write(
                            f"    Pewność: {segment.transcription.confidence:.2f}\n"
                        )
                        f.write(
                            f"    Czas przetwarzania: {segment.transcription.processing_time:.2f}s\n"
                        )
                    f.write("\n")
    ...
    def save_as_json(self, filename: str):
        ...
        for segment in self.transcriptions:
            segment_data = {
                ...
                "text": segment.text,
                "confidence": (
                    segment.transcription.confidence if segment.transcription else None
                ),
                "processing_time": (
                    segment.transcription.processing_time
                    if segment.transcription
                    else None
                ),
            }
    ...
    def load_audio_file(self):
        ...
        messagebox.showinfo(
            "Funkcja", "Przetwarzanie plików audio - w przyszłej wersji"
        )
...
class SettingsWindow:
    ...
    def create_interface(self):
        ...
        vad_frame = ttk.LabelFrame(
            main_frame, text="Voice Activity Detection", padding="10"
        )
        ...
        timing_frame = ttk.LabelFrame(
            main_frame, text="Ustawienia czasowe", padding="10"
        )
        ...
        ttk.Checkbutton(
            ui_frame, text="Automatyczne zapisywanie", variable=self.auto_save_var
        ).pack(anchor=tk.W)

        self.show_confidence_var = tk.BooleanVar(value=self.config["show_confidence"])
        ttk.Checkbutton(
            ui_frame,
            text="Pokaż pewność transkrypcji",
            variable=self.show_confidence_var,
        ).pack(anchor=tk.W)

        self.show_timing_var = tk.BooleanVar(value=self.config["show_timing"])
        ttk.Checkbutton(
            ui_frame, text="Pokaż znaczniki czasu", variable=self.show_timing_var
        ).pack(anchor=tk.W)
        ...
        ttk.Button(button_frame, text="OK", command=self.save_settings).pack(
            side=tk.RIGHT, padx=5
        )
        ttk.Button(button_frame, text="Anuluj", command=self.window.destroy).pack(
            side=tk.RIGHT
        )
        ttk.Button(button_frame, text="Domyślne", command=self.reset_to_defaults).pack(
            side=tk.LEFT
        )
